...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (move_uploaded_file($_FILES["img"]["tmp_name"], $target_file)) {
    ...
    try {
      ...
      $query = "INSERT INTO `advocate_info` (`a_slno`, `a_regd`, `a_name`, `a_email`, `a_phn`, `a_pass`, `a_path`, `status`) VALUES (NULL, '$regd', '$name', '$email', '$contactno', '$password', '$target_file', '1')";
...
<hr>
<p class="mb-0">Now login your dashboard panel -> <a href="a_login.php">Login Now</a>.</p>
</div>
</div>';
      } else {
      ...
      echo '
          ...
          <hr>
          <p class="mb-0">Try again -> <a href="a_signup.php">SignUP Again</a>.</p>
          </div>
          </div>';
    }
  } else {
